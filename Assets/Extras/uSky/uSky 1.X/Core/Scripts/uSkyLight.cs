...
namespace uSky
{
	...
	public class uSkyLight : MonoBehaviour {	
		...
		[Range(0f,2f)][Tooltip ("Brightness of the Moon (directional light). If the Moon Intensity is at 0 (less then 0.01), the Moon light will auto disabled and always disabled at Day time")]
		...
		private GameObject sunLightObj {
			get{
				if(uSM != null){
					return (uSM.SunLight != null)? uSM.SunLight : null;
				}else 
					return null;
			}
		}
		...
		private GameObject moonLightObj {
			get{
				if(uSM != null){
					return (uSM.MoonLight != null)? uSM.MoonLight : null;
				}else 
					return null;
			}
		}
		...
		void SunAndMoonLightUpdate (){
			...
			if (moonLightObj != null) {
				if (moon_Light != null) {
					...
					moon_Light.enabled = (currentTime > 0.26f && currentTime < 0.74f || MoonIntensity < 0.01)?  false:true;
				}
			}	
		}
